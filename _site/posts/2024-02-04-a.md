...
There are plenty of online resources which present recursive versions of
Tarjan's bridge algorithm, but I couldn't find any iterative versions.  It
wasn't straightforward to come up with one, since there is some tricky state in
the algorithm regarding the way that _deferred_ nodes are processed.
...
components can also have bridges which would split off an additional component.

## Reading the graph

Before calling the `get_bridge()` function, there is a parsing problem.  For the
Advent of Code problem, the graph is given in a format like this:
